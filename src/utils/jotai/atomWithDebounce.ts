...
export default function atomWithDebounce<T>(
  ...
  const currentValueAtomUnderlying = atom(initialValue)
  const currentValueAtom = atom(get => get(currentValueAtomUnderlying))
  const isDebouncingAtom = atom(false)

  const debouncedValueAtom = atom(initialValue, (get, set, update: SetStateAction<T>) => {
    clearTimeout(get(prevTimeoutAtom))

    const prevValue = get(currentValueAtomUnderlying)
    const nextValue = typeof update === 'function' ? (update as (prev: T) => T)(prevValue) : update

    const onDebounceStart = () => {
      set(currentValueAtomUnderlying, nextValue)
